...
Robot_CL.plotopt = {'floorlevel', 2, 'workspace', [-10, 60, -10, 30, -5, 5], 'reach', 10,'tilesize',12};
% Teta pose(desejado)
...
for i=1:1:steps
    ...
    % Teta Pose Desejada
    teta_pose = teta_pose + pi*Dt;
    
    % New Velocitys(targuet - current)/Dt
    vx = (40 - T0_EE_CL.t(1,1))/Dt;
    vy = (20 - T0_EE_CL.t(2,1))/Dt;
    wz = (teta_pose - teta_k)/Dt;
    ...
    % next q = old + deta t * q_ <----> q(k+1)=q(k)+ Kp*q_(k)*Dt
    q = q + Kp*q_*Dt;
    ...
    x_data(i)=T0_EE_CL.t(1,1);
    y_data(i)=T0_EE_CL.t(2,1);
end
